...
describe("config", function()
   ...
   it("returns nil, error on config with bad syntax", function()
      ...
      assert.is_nil(conf)
      assert.equal("Couldn't load configuration from spec/configs/bad_config.luacheckrc: syntax error (line 2: syntax error near 'method_missing')", err)

      nest("spec/configs/project", function()
         local nested_conf, nested_err = config.load_config("spec/configs/bad_config.luacheckrc")
         assert.is_nil(nested_conf)
         assert.equal("Couldn't load configuration from ../../../spec/configs/bad_config.luacheckrc: syntax error (line 2: syntax error near 'method_missing')", nested_err)
      end)
   end)
